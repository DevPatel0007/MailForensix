...
export function Hero({ onOpenWorkspace }: { onOpenWorkspace?: () => void }) {
      ...
      <Container className="relative flex flex-col pt-28 pb-12 lg:h-full lg:pb-0">
        ...
        <div className="max-w-xl">
          ...
          <Reveal delay={240} className="mt-8 flex flex-wrap items-center gap-3">
            <button
              type="button"
              onClick={onOpenWorkspace ?? handleGoogleLogin}
            ...
            <button
              ...
              className="inline-flex items-center gap-2.5 rounded-sm border border-hairline bg-background/60 px-5 py-3 text-sm font-medium text-foreground transition-colors duration-200 hover:border-steel disabled:opacity-50"
            >
              <GoogleIcon />
              {googleAuth.isFetching ? 'Connecting to Google...' : 'Sign up with Google'}
            </button>
          </Reveal>
        </div>
