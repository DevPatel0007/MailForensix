...
export function ProviderButton({ children, icon }: { children: ReactNode; icon: ReactNode }) {
  return (
    <button
      type="button"
      className="flex h-10 w-full items-center justify-center gap-2 rounded-md border border-hairline bg-surface text-[14px] font-medium text-foreground transition-colors duration-200 hover:border-steel hover:bg-secondary"
    >
      {icon}
      {children}
    </button>
