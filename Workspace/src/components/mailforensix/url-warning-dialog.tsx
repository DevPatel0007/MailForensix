...
import { AlertTriangle, ShieldAlert, ShieldCheck, ExternalLink, Copy } from 'lucide-react';
import { toast } from 'sonner';

export function UrlWarningDialog() {
  const { isUrlWarningOpen, warningUrlData, closeUrlWarning } = useMail();

  if (!warningUrlData) return null;

  const copyUrl = () => {
    navigator.clipboard.writeText(warningUrlData.url);
    toast.success('URL copied to clipboard');
  };

  const handleProceed = () => {
    closeUrlWarning();
    window.open(warningUrlData.url, '_blank', 'noopener,noreferrer');
  };

  const isDangerous = warningUrlData.status === 'MALICIOUS' || warningUrlData.status === 'SUSPICIOUS';

  return (
    <Dialog open={isUrlWarningOpen} onOpenChange={closeUrlWarning}>
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-3 py-2 text-xs">
          ...
          {warningUrlData.reasons && warningUrlData.reasons.length > 0 && (
            <div className="p-3 rounded-md bg-red-50/60 dark:bg-red-950/20 border border-red-200/70 dark:border-red-900/40 text-red-900 dark:text-red-200 space-y-1.5">
              ...
              <ul className="list-disc list-inside space-y-0.5 text-[11px] text-red-800 dark:text-red-300">
                {warningUrlData.reasons.map((r, i) => (
                  <li key={i}>{r}</li>
                ))}
              </ul>
            </div>
